...
julia +dyad-3.0.0 --project -e 'using Pkg; Pkg.instantiate()'
```
...
```bash
julia +dyad-3.0.0 --project scripts/generate_training_data.jl  # regenerate sin training set
julia +dyad-3.0.0 --project scripts/train.jl                   # Adam → LBFGS
```
...
```bash
julia +dyad-3.0.0 --project -e 'using Pkg; Pkg.test()'
```
