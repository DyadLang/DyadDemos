...
The default road is a continuous ISO 8608 roughness profile, with configurable
roughness and vehicle speed. Two-second plots show repeated suspension excitation
without compressing the response into a long trace. A single 3 cm bump remains
available with `road_profile="bump"`.
...
bump_road = Story.A4Performance(; road_profile="bump")
display(artifacts(bump_road, :SimulationSolutionPlot))
```
...
A5 uses the training excitation: 3 cm amplitude at 2 Hz, and plots the actual
saved `truck_sin_full_train.csv` acceleration samples as its reference. These are
synthetic data, not measured vehicle data. When excitation changes, the time range
exceeds the CSV, or the simulation starts after zero, it generates a matching
nonlinear reference instead; the plot identifies the source. The thicker reference
curve remains visible beneath the thinner learned prediction. A6 defaults to the
