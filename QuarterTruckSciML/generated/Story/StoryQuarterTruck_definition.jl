...
@doc Markdown.doc"""
   StoryQuarterTruck(; name, n_input, n_output, nn_depth, nn_width, chain, s_rel0_tire, s_rel0_seat, s_scale_tire, v_friction_scale, v_seat_scale, f_scale_tire, f_scale_friction)
...
"""
@component function StoryQuarterTruck(; name = nothing, n_input=3, n_output=3, nn_depth=1, nn_width=5, s_rel0_tire=0.2, s_rel0_seat=0.1, s_scale_tire=0.05, v_friction_scale=0.1, v_seat_scale=0.2, f_scale_tire=Float64(1000.0), f_scale_friction=Float64(500.0), chain=QuarterTruckSciML.DyadModelDiscovery.multi_layer_feed_forward(n_input, n_output; depth=nn_depth, width=nn_width, activation=QuarterTruckSciML.Lux.tanh), kwargs...)
  isnothing(name) && throw(ArgumentError("""
    The `name` keyword must be provided. Please consider using the `@named` macro,
    like so:
  
    @named model = StoryQuarterTruck()
  """))
  ...
  push!(__systems, @named road = TranslationalComponents.Sources.Position(; ref_type=QuarterTruckSciML.TranslationalComponents.Sources.ReferenceType.Filtered(; f_crit=Float64(100.0)), road_overrides...))
