...
Hoffmann's spec, 2026-04-27):
    ...
    model.body_m          = 300.0 → 315.0  (+5%,  mass)
    model.tire_to_body_c  = 20e3  → 22e3   (+10%, stiffness)
    model.tire_to_body_d  = 1500  → 1800   (+20%, damping)
    model.friction_Fc     = 500.0 → 750.0  (+50%, friction)
...
Output: `assets/data/calibration_measurements.csv` (1001 rows over 10 s @ 100 Hz).
=#
...
# system — these are the same top-level knobs the calibration analysis searches
# over (see `search_space_names` in calibration.dyad). They must be the knobs,
# not the inner sub-component params (`body.m`, `tire_to_body.c`, …): those are
# `final`-bound to the knobs and have no overridable `Initial` parameter.
overrides = [
    sys.model.body_m         => 315.0,    # +5%
    sys.model.tire_to_body_c => 22e3,     # +10%
    sys.model.tire_to_body_d => 1800.0,   # +20%
    sys.model.friction_Fc    => 750.0,    # +50%
]
...
@info "Truth values to recover: body_m=315, tire_to_body_c=22000, tire_to_body_d=1800, friction_Fc=750"
