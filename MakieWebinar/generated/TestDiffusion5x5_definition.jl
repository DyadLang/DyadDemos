...
@component function TestDiffusion5x5(; name = nothing, N=5, N_caps=25, N_h_res=20, N_v_res=20, N_boundary=20)
  ...
  caps = System[]
  for i in 1:N_caps
    push!(caps, MakieWebinar.DiffusionCapacitor(V=0.01, name=Symbol("caps", "⸺", i)))
  end
  append!(__systems, caps)
  r_horiz = System[]
  for i in 1:N_h_res
    push!(r_horiz, MakieWebinar.DiffusionResistor(D=1, dx=0.1, A=0.1, name=Symbol("r_horiz", "⸺", i)))
  end
  append!(__systems, r_horiz)
  r_vert = System[]
  for i in 1:N_v_res
    push!(r_vert, MakieWebinar.DiffusionResistor(D=1, dx=0.1, A=0.1, name=Symbol("r_vert", "⸺", i)))
  end
