### DO NOT EDIT THIS FILE
### This file is auto-generated by the Dyad command-line compiler.
### If you edit this code it is likely to get overwritten.
### Instead, update the Dyad source code and regenerate this file


@doc Markdown.doc"""
   TestColdNightSizing(; name)
"""
@component function TestColdNightSizing(; name = nothing)
  isnothing(name) && throw(ArgumentError("""
        The `name` keyword must be provided. Please consider using the `@named` macro,
        like so:

        @named model = TestColdNightSizing()
        """))
  __params = Symbolics.SymbolicT[]
  __vars = Symbolics.SymbolicT[]
  __systems = System[]
  __guesses = Dict{Symbolics.SymbolicT, Symbolics.SymbolicT}()
  __initial_conditions = Dict{Symbolics.SymbolicT, Symbolics.SymbolicT}()
  __initialization_eqs = Equation[]
  __eqs = Equation[]

  ### Symbolic Parameters

  ### Variables

  ### Constants
  __constants = Any[]

  ### Components
  push!(__systems, @named building = DynamicSteadyState.ThreeZoneBuilding())
  push!(__systems, @named outdoor_signal = BlockComponents.Constant(k=263.15))
  push!(__systems, @named ground_signal = BlockComponents.Constant(k=283.15))
  push!(__systems, @named zone1_setpoint = ThermalComponents.FixedTemperature(T=294.15))
  push!(__systems, @named zone2_setpoint = ThermalComponents.FixedTemperature(T=294.15))
  push!(__systems, @named zone3_setpoint = ThermalComponents.FixedTemperature(T=294.15))

  ### Guesses

  ### Defaults

  ### Initialization Equations

  ### Assertions
  __assertions = []

  ### Equations
  push!(__eqs, connect(outdoor_signal.y, building.T_outdoor))
  push!(__eqs, connect(ground_signal.y, building.T_ground))
  push!(__eqs, connect(zone1_setpoint.node, building.zone1_port))
  push!(__eqs, connect(zone2_setpoint.node, building.zone2_port))
  push!(__eqs, connect(zone3_setpoint.node, building.zone3_port))

  # Return completely constructed System
  return System(__eqs, t, __vars, __params; systems=__systems, initial_conditions=__initial_conditions, guesses=__guesses, name, initialization_eqs=__initialization_eqs, assertions=__assertions)
end
export TestColdNightSizing
