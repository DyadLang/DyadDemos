...
@testset "Running test case1 for ASPDemo.TestSourceSink" begin
  using CSV, DataFrames, Plots
  using DyadInterface: TransientAnalysis, rebuild_sol, ODEAlg
  using ModelingToolkit: toggle_namespacing, get_initial_conditions, @named

  @named model = ASPDemo.TestSourceSink()
...
# Signals selected for regression testing: ["source.port.m_flow","source.port.h_outflow"]
  ...
  if get(ENV, "DYAD_UPDATE_REFS", "") !== ""
    ...
    mkpath("snapshots")
    CSV.write("snapshots/TestSourceSink_case1_sig0.ref", DataFrame(t=ref_times, expected=[sol(t, idxs=model.source.port.m_flow) for t in ref_times]))
    CSV.write("snapshots/TestSourceSink_case1_sig1.ref", DataFrame(t=ref_times, expected=[sol(t, idxs=model.source.port.h_outflow) for t in ref_times]))
  end
    if isfile("snapshots/TestSourceSink_case1_sig0.ref")
      ...
      if get(ENV, "DYAD_COMPARISONS", "") !== ""
        ...
        mkpath("comparisons")
        savefig("comparisons/TestSourceSink_case1_sig0.png")
      end
    else
      mkpath("snapshots")
      CSV.write("snapshots/TestSourceSink_case1_sig0.ref", DataFrame(t=ref_times, expected=[sol(t, idxs=model.source.port.m_flow) for t in ref_times]))
    ...
    if isfile("snapshots/TestSourceSink_case1_sig1.ref")
      ...
      if get(ENV, "DYAD_COMPARISONS", "") !== ""
        ...
        mkpath("comparisons")
        savefig("comparisons/TestSourceSink_case1_sig1.png")
      end
    else
      mkpath("snapshots")
      CSV.write("snapshots/TestSourceSink_case1_sig1.ref", DataFrame(t=ref_times, expected=[sol(t, idxs=model.source.port.h_outflow) for t in ref_times]))
