...
@testset "Running test case1 for ASPDemo.TestClarifierBottom" begin
  using CSV, DataFrames, Plots
  using DyadInterface: TransientAnalysis, rebuild_sol, ODEAlg
  using ModelingToolkit: toggle_namespacing, get_initial_conditions, @named

  @named model = ASPDemo.TestClarifierBottom()
...
# Signals selected for regression testing: ["volume.T","resistance.port_a.m_flow"]
  ...
  if get(ENV, "DYAD_UPDATE_REFS", "") !== ""
    ...
    mkpath("snapshots")
    CSV.write("snapshots/TestClarifierBottom_case1_sig0.ref", DataFrame(t=ref_times, expected=[sol(t, idxs=model.volume.T) for t in ref_times]))
    CSV.write("snapshots/TestClarifierBottom_case1_sig1.ref", DataFrame(t=ref_times, expected=[sol(t, idxs=model.resistance.port_a.m_flow) for t in ref_times]))
  end
    if isfile("snapshots/TestClarifierBottom_case1_sig0.ref")
      ...
      if get(ENV, "DYAD_COMPARISONS", "") !== ""
        ...
        mkpath("comparisons")
        savefig("comparisons/TestClarifierBottom_case1_sig0.png")
      end
    else
      mkpath("snapshots")
      CSV.write("snapshots/TestClarifierBottom_case1_sig0.ref", DataFrame(t=ref_times, expected=[sol(t, idxs=model.volume.T) for t in ref_times]))
    ...
    if isfile("snapshots/TestClarifierBottom_case1_sig1.ref")
      ...
      if get(ENV, "DYAD_COMPARISONS", "") !== ""
        ...
        mkpath("comparisons")
        savefig("comparisons/TestClarifierBottom_case1_sig1.png")
      end
    else
      mkpath("snapshots")
      CSV.write("snapshots/TestClarifierBottom_case1_sig1.ref", DataFrame(t=ref_times, expected=[sol(t, idxs=model.resistance.port_a.m_flow) for t in ref_times]))
